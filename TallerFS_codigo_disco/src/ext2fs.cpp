...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	Ext2FSSuperblock* s_bloque = superblock();
	unsigned int block_size = 1024 << superblock()->log_block_size;
	int inodes_per_block = block_size / s_bloque->inode_size;
	Ext2FSInode buffer[inodes_per_block];

	unsigned int grupo_del_inodo = blockgroup_for_inode(inode_number);
	unsigned int indice_inodo_en_grupo = blockgroup_inode_index(inode_number);

	Ext2FSBlockGroupDescriptor* desc_grupo = block_group(grupo_del_inodo);
	unsigned int inicio_tabla_inodos = desc_grupo->inode_table;
	unsigned int block_number = inicio_tabla_inodos + indice_inodo_en_grupo/inodes_per_block;
	read_block(block_number, (unsigned char *) buffer);

	Ext2FSInode* inodo = new Ext2FSInode();
	*inodo = buffer[indice_inodo_en_grupo%inodes_per_block];
	return inodo;
}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	unsigned int dirs_en_bloque = block_size / 4;

	if(block_number > inode->blocks){
		res = 0;
	}
	...
	unsigned int dirs_hasta_simple = dirs_en_bloque+12;
	if(block_number >= 12 && block_number < dirs_hasta_simple){
		unsigned int indirecto_simple = inode->block[12];
		read_block(indirecto_simple, (unsigned char*) buffer);

		res = buffer[(block_number - 12)];
		//res = ((unsigned int*) indirecto_simple)[block_number -12];
	}

	unsigned int dirs_en_doble = dirs_en_bloque*dirs_en_bloque;
	unsigned int dirs_hasta_doble = dirs_hasta_simple + dirs_en_doble;
	if(block_number >= dirs_hasta_simple && block_number < dirs_hasta_doble){

		unsigned int block_number_doble = block_number - dirs_hasta_simple;
		
		unsigned int indirecto_doble = inode->block[13];
		read_block(indirecto_doble, (unsigned char*) buffer);

		unsigned int num_bloque_indirecto = block_number_doble/dirs_en_bloque;
		unsigned int indirecto_simple = buffer[num_bloque_indirecto];
		read_block(indirecto_simple, (unsigned char*) buffer);
		//unsigned int indirecto_simple = ((unsigned int*) indirecto_doble)[num_bloque_indirecto];
		
		res = buffer[block_number_doble%dirs_en_bloque ];

	}

	if(block_number >= dirs_hasta_doble){
		unsigned int indirecto_triple = inode->block[14];
		read_block(indirecto_triple, (unsigned char*) buffer);

		unsigned int num_bloque_doble = (block_number-dirs_hasta_doble)/dirs_en_doble;
		unsigned int indirecto_doble = buffer[num_bloque_doble];
		read_block(indirecto_doble, (unsigned char*) buffer);

		unsigned int num_bloque_indirecto = ((block_number-dirs_hasta_doble)/dirs_en_bloque)%dirs_en_bloque;
		unsigned int indirecto_simple = buffer[num_bloque_indirecto];
		read_block(indirecto_simple, (unsigned char*) buffer);
		
		res = buffer[(block_number -12)%dirs_en_bloque ];
	}
